...
async function readPickedAttachment(
  ...
  const textContent = normalizeText(await FileSystem.readAsStringAsync(asset.uri));
  ...
  const truncatedText =
    textContent.length > MAX_ATTACHMENT_CHARACTERS
      ? `${textContent.slice(0, MAX_ATTACHMENT_CHARACTERS)}\n\n[Содержимое файла было сокращено приложением, чтобы не перегружать чат.]`
      : textContent;
...
function validateAsset(asset: DocumentPicker.DocumentPickerAsset) {
  ...
  if (typeof asset.size === "number" && asset.size > MAX_ATTACHMENT_BYTES) {
    throw new Error(
      `Файл "${asset.name}" слишком большой. Для чата сейчас поддерживаются текстовые файлы до ${Math.round(
