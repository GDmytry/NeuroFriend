...
export function ChatExperienceScreen({ navigation }: Props) {
  ...
  async function handleSend() {
    ...
    const previewMessage: ChatMessage = {
      id: `pending-${Date.now()}`,
      author: "user",
      text: buildPendingPreviewText(trimmedDraft, pendingAttachments),
          ...
          >
                ...
                {visibleMessages.length === 0 ? (
                  <View style={styles.emptyWrap}>
                    ...
                    <Text style={styles.emptySubtitle}>
                      {assistantName} ждет ваше сообщение. Новый чат, режимы и история доступны
            ...
            {drawerMounted ? (
              <View style={StyleSheet.absoluteFill}>
                <Animated.View
                  pointerEvents="none"
                  style={[styles.dimmer, { opacity: dimmerOpacity }]}
                />
...
function buildPendingPreviewText(text: string, attachments: ChatAttachment[]) {
  if (text.trim()) {
    return text.trim();
  }
